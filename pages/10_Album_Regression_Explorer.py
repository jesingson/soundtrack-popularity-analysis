...
def render_residual_diagnostics(
    ...
    """
    Render regression residual Q-Q diagnostics.
    ...
    chart = create_qq_plot(
        values=residuals,
        chart_title=f"Q-Q Plot: Regression Residuals for {get_display_label(target_col)}",
        y_label="Residuals",
    )
    st.altair_chart(chart, width="stretch")
    ...
    if r >= 0.99:
        ...
        takeaway = (
            "Residuals are broadly close to normal, though there may still be mild tail "
            "deviations."
        )
    elif r >= 0.94:
        takeaway = (
...
def render_target_qq_comparison(
    filtered_album_df: pd.DataFrame,
    target_col: str,
) -> None:
    """
    Render a side-by-side Q-Q comparison for the raw and modeled target.

    Args:
        filtered_album_df: Filtered album dataframe used for modeling.
        target_col: Current modeled target column.
    """
    raw_target_map = {
        "log_lfm_album_listeners": "lfm_album_listeners",
        "log_lfm_album_playcount": "lfm_album_playcount",
    }

    raw_target_col = raw_target_map.get(target_col)

    if not raw_target_col or raw_target_col not in filtered_album_df.columns:
        return
    ...
    with left:
        raw_chart = create_qq_plot(
            values=filtered_album_df[raw_target_col],
            chart_title=f"Q-Q Plot: Raw {get_display_label(raw_target_col)}",
            y_label=get_display_label(raw_target_col),
        )
        st.altair_chart(raw_chart, width="stretch")

    with right:
        log_chart = create_qq_plot(
            values=filtered_album_df[target_col],
            chart_title=f"Q-Q Plot: {get_display_label(target_col)}",
            y_label=get_display_label(target_col),
        )
        st.altair_chart(log_chart, width="stretch")

    raw_clean = pd.to_numeric(filtered_album_df[raw_target_col], errors="coerce").dropna()
    log_clean = pd.to_numeric(filtered_album_df[target_col], errors="coerce").dropna()
    ...
    if log_r > raw_r:
        st.caption(
            ...
            f"{get_display_label(target_col)} in the regression."
        )
    else:
        st.caption(
...
def main() -> None:
    ...
    if controls["show_residual_qq"]:
        render_residual_diagnostics(
            results=results,
            target_col=target_col,
        )

    if controls["show_target_qq"]:
        render_target_qq_comparison(
            filtered_album_df=filtered_album_df,
            target_col=target_col,
        )
